function [ut] = aircraftMPC_MS(dt_ctrl, dt_simu, dt_iter, mu, sigma, R)
% clear
% close all
% dt_ctrl = 0.2;
% dt_simu = 0.02;
...
% velocity
x0 = [linspace(mu(1),mu(1)+1.5*v_min*cos(mu(3))*horizon,gridN)'; linspace(mu(2),mu(2)+1.5*v_min*sin(mu(3))*horizon,gridN)'; mu(3)*ones(gridN,1); 1.5*v_min*ones(gridN, 1);zeros(gridN, 1)];
...
options = optimoptions(@fmincon, 'TolFun', 0.000001, 'MaxIter', 50, ...
                       'MaxFunEvals', 20000,  ...
                       'DiffMinChange', 0.001, 'Algorithm', 'sqp');
